library('optparse')
option_list <- list(
	make_option(c("-i", "--feature-table-with-comment"),metavar="path", dest="data",help="Specify the abundance table you want to debatch, you should not contain any comment in the file, taxonomy or otuid must in the firsr as well as last column",default=NULL),
	make_option(c('-f',"--feature-table-without-comment"),metavar="path", dest="otu",help="Specify the otu table you want to debatch with comment at first row, if --input is supplied, this one will be ignored.",default=NULL),
...
if (!is.null(opt$group)&N>0){
  write.table(data.frame(map,batch_map,Description=map[,1],check.names = F,check.rows = F),paste(opt$out,'/','Batch_corrected_map.txt',sep = ''),sep = '\t',quote = F,row.names = F)
}
