...
if (!is.null(opt$group)){
  ...
  for (group in str_split(opt$group,',')[[1]]) {
    ...
    batch_map <- ComBat(dat=batch_map, batch=batch, mod=modcombat, par.prior=F, prior.plots=F,mean.only = as.logical(opt$onlymean))
...
if(!is.null(opt$data)){
  write.table(data,paste(opt$out,'/','Batch_corrected_feature_table.txt',sep = ''),sep = '\t',quote = F,row.names = F)
}else{
  cm<-"#Feature table filterd or debatched"
  write.table(cm,paste(opt$out,'/','Batch_corrected_feature_table.txt',sep = ''),sep = '\t',quote = F,row.names = F,col.names=F,append = F)
  write.table(data,paste(opt$out,'/','Batch_corrected_feature_table.txt',sep = ''),sep = '\t',quote = F,row.names = F,append = T)
}

if (!is.null(opt$group)){
  write.table(data.frame(map,batch_map,check.names = F,check.rows = T),paste(opt$out,'/','Batch_corrected_map.txt',sep = ''),sep = '\t',quote = F,row.names = F)
}
