...
option_list <- list( 
  ...
  make_option(c("-e", "--exclude"),metavar="string",dest="ex", help="Specify the numeric variables excluded from calculate mean",default="none"),
...
if(!is.null(opt$spe)){
  spe<-read.table(opt$spe,header = T, comment.char = "",sep = "\t",check.names = F,stringsAsFactors = F)
#  spe<-read.table("otu_table.Genus.absolute.txt",header = T, comment.char = "",sep = "\t",check.names = F,stringsAsFactors = F)
}else{
  if(!is.null(opt$otu)){
    spe<-read.table(opt$otu,skip = 1, header = T, comment.char = "",sep = "\t",check.names = F,stringsAsFactors = F)
    }
}

map<-read.table(opt$map,row.names = 1,na.strings = "",header = T, comment.char = "",sep = "\t",check.names = F,stringsAsFactors = F)
#map<-read.table("mapping_file.txt",row.names = 1,na.strings = "",header = T, comment.char = "",sep = "\t",check.names = F,stringsAsFactors = F)

ifspe<-tryCatch({lspe<-ncol(spe);map<-map[match(colnames(spe)[-c(1,lspe)],rownames(map)),];TRUE},error=function(e){write("####Only collapse map###",stdout());return(FALSE)})
